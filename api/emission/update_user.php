...
if(
    ...
    try {
        ...
        if(!$user->findByPhone()) {
            throw new Exception("User not found with phone number: " . $data->phone);
        }
        ...
        // Обновляем поле emission_kg пользователя
        $update_query = "UPDATE users SET emission_kg = ? WHERE id = ?";
        $update_stmt = $db->prepare($update_query);
        $update_stmt->bindParam(1, $total_emission);
        $update_stmt->bindParam(2, $user->id);
        
        if(!$update_stmt->execute()) {
            throw new Exception("Failed to update user emission data");
        }
